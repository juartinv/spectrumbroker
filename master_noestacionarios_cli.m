...
#   juan vanerio, 2017
#
# Este script se usa para la etapa final, para evaluar los mecanismos seleccionados y la politica 
# calculada por MDP (parametros obtenidos por MLE sobre cada plan de ventos completo) para el tipo 
# de oponente olvidadizo que se indique (arg. de entrada "tipo").
#
# El flujo de trabajo a seguir para esta simulación es:
#    1 - ejecutar generar_schedules_alternativos.m para generar 21 planes de eventos de categoria A, 
#        21 de cat.B y 21 de cat.C , y con los planes de eventos determinar los parametros MLE que estiman
#        como procesos poisson/exponencial los comportamientos de los usuarios. 
#        Crea un archivo "noestacionarios/ev_scheds.mat" con una variable "hat_theta" con parametros Poisson/exp
#        en cada fila y una variable "M_sched" con 63 planes de evento para cada tipo de oponente.
#    2 - ejecutar policyIterator_script_multiples_noestacionarios.m que lee el archivo creado antes y calcula las 
#        politicas MDP optimas para cada conjunto de parametros estimado. Los resultados se guardan en el archivo 
#        "param_resultado.mat" ("M_sched_sel" (un subconjunto de M_sched),"q" (identico a hat_theta),"policies" 
...
if (nargin != 2)
    error("Uso desde la shell de linux: octave-cli -q master_noestacionarios_cli.m  <tipo:1,2,3,4,5> <filename.mat>")
endif
arg_list = argv ();
#######################################################################################
###  inicializo sistemas
...
tipo=str2num(arg_list{1})         #maximal, saturada, borde, total
...
load /home/jvanerio/Escritorio/tesis/noestacionarios/param_resultado.mat
...
for MM=1:iteraciones,
    ...
    T=1000              #aproximated system elapsed time (in same units as lambda). Variable dummy.
