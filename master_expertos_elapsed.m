...
#   juan vanerio, 2017
#
# Este script se utiliza para evaluar el tiempo minimo requerido en la ejecución de los 
# distintos mecanismos de prediccion al variar la cantidad de expertos considerados. 
# La idea es que de esta forma puede aproximarse el costo computacional asociado a dicha 
# variación. Se utiliza un unico juego de parametros y se genera un plan de eventos acorde
# para realizar los ensayos.
# 
# Los resultados y se procesan  y grafican inmediatamente
...
#global programas=[ 201 202 203 204]+60
#theta=2;
#global fparam_global=ones(size(programas))*theta
...
parametros_lowb=[T 35 40  0.08  1];  %recirdar que mu1 esta en escala logaritmica...
...
function [datos, payoff_evol] = spectrum_auction(indice)
    ...
    #programas={'dual_complete','regular','dual','delayed','expertbystate'};
...
#plot(parciales(:,9)-1,parciales(:,8),'x')
#ylabel('elapsed time (s)')
#xlabel('N')
#grid
#title('Tiempo Transcurrido en funcion de la cantidad de expertos considerados')
...
#for i=1:iteraciones,
    for p=1:numparams,
        indices = find (m_aug(:,2)==programas(p)) ;
        programa = floor(programas(p)/10) ;
        expertos = 4*(1+mod(programas(p),10) );
        
        [medidas,ind] = sort( m_aug(indices,4));
        ...
        estadistica_summ=[estadistica_summ; [programa expertos datos] ];  %reordeno los datos par uqe quede priemro tol, luego notol, luego diff
    endfor
#endfor
...
title('Tiempo Transcurrido en funcion de la cantidad de expertos considerados')
ylabel("Duracion (s)")
xlabel("Cantidad de Expertos")
labels={};
for p=1:l,
    indices=find((estadistica_summ(:,1)==progcode(p)));


    h=plot(estadistica_summ(indices,2), estadistica_summ(indices,3));
